...
| q-download-manager | Download manager (save fetched resource to disk, progress, list) | Open. No mechanism today — file input (Q-28) is upload-only, nothing writes fetched bytes to disk on user request. |
...
| q-dev-tools | Dev tools (console/network/DOM inspector) | Open. Today: Ctrl-D box-outline + focus-order overlay only (Q-12/Q-22). No console, no network log, no live DOM tree view. |
| q-extensions-api | Extensions API | Open. No plugin/extension surface exists. Would need a stable extension point in `client/` (menu, render, or net layer) — undefined today. |
...
| q-websocket-support | WebSocket support | Open. `net/` is HTTP/1.1 request-response only (`cpp-httplib`). WebSocket needs a new client dependency or protocol upgrade path — undefined today. |
| q-multiprocess-sandboxing | Multi-process / sandboxing between tabs | Open. Client is single-process; one tab's fault can affect the whole app. Multi-process would be a significant architecture change (IPC, process-per-tab) vs current single-binary `TApplication` model. |
