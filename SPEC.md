...
| q-text-selection | Text selection (mouse/keyboard range-select + copy) | **Implemented**: extends Q-5 — mouse-drag range-selection (reverse-video overlay), Ctrl-C copies selection when active else falls back to link-URL copy. Keyboard range-extend (shift+arrow) not implemented — scope cut, see `adr-text-selection`. |
